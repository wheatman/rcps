...
/* rcps random-based brute-forcer simulation. randomizes states and sees how
long the cog stays still for. the c++ is a significant improvement*/
...
for(a = 0; a < 12; a++){
	...
	printf("Pusher %i countdown: %i \n",a+1, inputstate->pushers[a].countdown);
...
for(a = 0; a < 4; a++){
	printf("Cogs %i currentAngularVelocity: %i \n",a+1, inputstate->cogs[a].currentAngularVelocity);
	printf("Cogs %i targetAngularVelocity: %i \n",a+1, inputstate->cogs[a].targetAngularVelocity);
} for(a = 0; a < 2; a++){
	printf("Spinningtriangles %i currentAngularVelocity: %i \n",a+1, inputstate->spinningtriangles[a].currentAngularVelocity);
	printf("Spinningtriangles %i targetAngularVelocity: %i \n",a+1, inputstate->spinningtriangles[a].targetAngularVelocity);
...
for(a = 0; a < 2; a++){
	printf("Bobomb %i blinking timer: %i \n",a+1, inputstate->bobombs[a].blinkingTimer);
}
}
...
void runsimulation(){
	printf("Running...\n");
	initialize_rand();
	objects_t* states = malloc(100 * sizeof(*states));
	objects_t *currentstartingarray = malloc(100 * sizeof(*currentstartingarray));
	int a;
	int b = 0;
	while(b == 0){
		randomizearray(states);
		currentstartingarray = states;
		printf("Initial states:\n");
		printobjectstates(currentstartingarray);
		cogstill = 1;
		for(a = 0; a < 1200; a++){
			advanceobjects(states);
			if(cogstill == 0) a = 1200;
		} if(cogstill == 1){
			printf("Succesful state:\n");
			printobjectstates(currentstartingarray);
			b = 1;
		}
	}
